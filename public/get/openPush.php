...
if (is_numeric($id)) {
    ...
    if ($read->getResult()) {
        ...
        if($read->getResult()) {
            ...
            $up->exeUpdate("enviar_mensagem", ["total_de_conversao" => $nTotal, "taxa_de_conversao" => (($nTotal * 100) / $m['total_de_entrega'])], "WHERE id = :eid", "eid={$notificacao['enviar_mensagem_id']}");
        }
    }
}